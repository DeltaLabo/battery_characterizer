'''
Este programa varía en relación al cycler_siglent.py debido a que este sí 
generará una descarga completa ahora no solamente llegando hasta los 2.5V, 
sino que también llegará la corriente hasta 100 mA
...
def INIT(entry):
 ...
 _!`                      _GEy)#@@@#O33Z#gP3H8@@@@Z^ZEY`                    `:" 
  `~r(~_`                :ZEx}@@RPPMB@@@@@@@@$PPMB@Q^KDV`              `-=)r^'  
    ...
    if input("Desea realizar solo carga (y/n): \n") == 'y':
        charge_only = 1
        print("Solo CARGA...")
    state = "CHARGE"
    init_flag = 1
